...
const GraphCanvasContent = ({
  ...
  useEffect(() => {
    if (initialNodes && initialNodes.length > 0) {
      ...
      const rawNodes = initialNodes.map((n) => {
        ...
        return {
          id: n.id,
          data: { label: n.label || n.id, ...n, color: nodeColor },
      ...
      const rawEdges = initialEdges.map(e => ({
        ...
        type: 'default', // Use our custom edge component
        data: { ...e, type: e.type }, // Pass type and other data to CustomEdge
  ...
  const onConnect = useCallback(async (params) => {
    // Check if this is a Parent-Child connection
    const isHierarchyConnection =
      (params.sourceHandle === 'bottom' && params.targetHandle === 'top') ||
      (params.sourceHandle === 'top' && params.targetHandle === 'bottom'); // Less common but handle it

    if (isHierarchyConnection) {
      // Auto-create "contains" edge
      try {
        await createEdge(params.source, params.target, "contains");
        if (onRefresh) onRefresh();
      } catch (error) {
        console.error("Failed to create hierarchy edge:", error);
      }
    } else {
      // Standard Reference connection -> Delegate to App (Modal)
      if (onConnectRequest) {
        onConnectRequest(params);
      }
    }
  }, [onConnectRequest, onRefresh]);
        ...
        <div className="bg-gray-900/80 backdrop-blur border border-white/10 p-1.5 rounded-xl shadow-xl flex gap-1 items-center">
          {['F0', 'F1', 'F2'].map((mode) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${depthMode === mode
